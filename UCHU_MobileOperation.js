...
@param DPad On Image
@desc 方向パッド押下時画像のファイルパス
@default ./img/system/DirPad_On.png

@param ActionBtn On Image
@desc 決定ボタン押下時画像のファイルパス
@default ./img/system/ActionButton_On.png

@param CancelBtn On Image
@desc キャンセル（メニュー）ボタン押下時画像のファイルパス
@default ./img/system/CancelButton_On.png
...
(function() {
	...
	PRM.url[3] = String(Parameters["DPad On Image"]);
	PRM.url[4] = String(Parameters["ActionBtn On Image"]);
	PRM.url[5] = String(Parameters["CancelBtn On Image"]);
	...
	Scene_Base.prototype.start = function() {
	    ...
	    if (Utils.isMobileDevice() || PRM.pcBtn) {
			if(!Btn_ready){
				Btn_ready=true;
				var setTouchEvent = function (element, src, offBg, onBg) {
					var img = new Image();
					img.src = src;
					element.addEventListener('touchstart', function () {
						this.style.background = onBg;
					}, false);
					element.addEventListener('touchend', function () {
						this.style.background = offBg;
					}, false);
				};
				if (PRM.visible[0]) {
					this.DirPad = new Locate_DirPad();
					setTouchEvent(
						this.DirPad.Div,
						PRM.url[3],
						'url('+PRM.url[0]+') 50% 50% / '+String(Math.round(PRM.size[0]/pad_range*100))+'% no-repeat',
						'url('+PRM.url[3]+') 50% 50% / '+String(Math.round(PRM.size[0]/pad_range*100))+'% no-repeat'
					);
				}
				if (PRM.visible[1]) {
					this.okButton = new Locate_Button(1);
					setTouchEvent(
						this.okButton.Div,
						PRM.url[4],
						'url('+PRM.url[1]+') 0 0 / cover no-repeat',
						'url('+PRM.url[4]+') 0 0 / cover no-repeat'
					);
				}
				if (PRM.visible[2]) {
					this.canselButton = new Locate_Button(2);
					setTouchEvent(
						this.canselButton.Div,
						PRM.url[5],
						'url('+PRM.url[2]+') 0 0 / cover no-repeat',
						'url('+PRM.url[5]+') 0 0 / cover no-repeat'
					);
				}
